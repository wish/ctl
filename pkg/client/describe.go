...
func (c *Client) DescribeConfigMap(configmap types.ConfigMapDiscovery) (string, error) {
  d, err := c.getDescriber(configmap.Context, schema.GroupKind{Group: batchv1.GroupName, Kind: "ConfigMap"})
  if err != nil {
    return "", err
  }

  s, err := d.Describe(configmap.Namespace, configmap.Name, describe.DescriberSettings{})
...
func (c *Client) DescribeDeployment(deployment types.DeploymentDiscovery) (string, error) {
  d, err := c.getDescriber(deployment.Context, schema.GroupKind{Group: appsv1.GroupName, Kind: "Deployment"})
  if err != nil {
    return "", err
  }

  s, err := d.Describe(deployment.Namespace, deployment.Name, describe.DescriberSettings{})
...
func (c *Client) DescribeReplicaSet(replicaset types.ReplicaSetDiscovery) (string, error) {
  d, err := c.getDescriber(replicaset.Context, schema.GroupKind{Group: appsv1.GroupName, Kind: "ReplicaSet"})
  if err != nil {
    return "", err
  }

  s, err := d.Describe(replicaset.Namespace, replicaset.Name, describe.DescriberSettings{})
